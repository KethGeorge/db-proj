<template>
    <div class="container">
        <Breadcrumb
            :items="['NationalStandard', isCreate ? 'nationalStandard.form.title.create' : (isEdit ? 'nationalStandard.form.title.edit' : 'nationalStandard.form.title.view')]" />
        <a-form ref="formRef" layout="vertical" :model="formData" @submit="onSubmitClick">
            <a-space direction="vertical" :size="16">
                <a-card class="general-card" :bordered="false">
                    ...
                    <a-row :gutter="16">
                        <a-col :span="12">
                            <a-form-item :label="$t('nationalStandard.form.label.NSN')" field="NSN"
                                :rules="[{ required: true, message: $t('nationalStandard.form.validation.NSNRequired') }]">
                                <a-input v-model="formData.NSN"
                                    :placeholder="$t('nationalStandard.form.placeholder.NSN')" :disabled="!isCreate" />
                                <!-- NSN 仅在创建时可编辑 -->
                            </a-form-item>
                        </a-col>
                        <a-col :span="12">
                            <a-form-item :label="$t('nationalStandard.form.label.StandardName')" field="StandardName"
                                :rules="[{ required: true, message: $t('nationalStandard.form.validation.StandardNameRequired') }]">
                                <a-input v-model="formData.StandardName"
                                    :placeholder="$t('nationalStandard.form.placeholder.StandardName')"
                    ...
                    <a-row :gutter="16">
                        <a-col :span="12">
                            <a-form-item :label="$t('nationalStandard.form.label.MaterialCode')" field="MaterialCode">
                                <a-input v-model="formData.MaterialCode"
                                    :placeholder="$t('nationalStandard.form.placeholder.MaterialCode')"
                                    :disabled="!isEdit && !isCreate" />
                            </a-form-item>
                        </a-col>
                        <!-- 可以添加其他字段，例如 Description 的 Textarea -->
                        <a-col :span="12">
                            <a-form-item :label="$t('nationalStandard.form.label.Description')" field="Description">
                                <a-textarea v-model="formData.Description"
                                    :placeholder="$t('nationalStandard.form.placeholder.Description')"
                                    :disabled="!isEdit && !isCreate" :auto-size="{ minRows: 2, maxRows: 5 }" />
                            </a-form-item>
                        </a-col>
                    </a-row>
...
// 导入统一管理后的 API 函数
import { queryNationalStandardDetail, createNationalStandard, updateNationalStandardInfo, NationalStandardRecord } from '@/api/NationalStandard';
...
const fetchStandardData = async (nsn: string) => {
    ...
    try {
        ...
        Message.error(`${t('nationalStandard.message.fetchFail')}: ${error.message || '未知错误'}`);
    ...
    if (!errors) {
        ...
        try {
            ...
            if (isCreate.value) {
                ...
                res = await updateNationalStandardInfo(nsnFromRoute.value, dataToSend);
