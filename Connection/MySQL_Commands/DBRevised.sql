CREATE DATABASE  IF NOT EXISTS `devexp` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `devexp`;
...
INSERT INTO `device` VALUES ('DEV001','黄昏',NULL,NULL,NULL,NULL),('DEV002','微型粒子对撞机','制造重氢和反物质','2023-02-01 09:00:00','2023-07-01 14:00:00',NULL),('DEV003','离心机','分离','2023-03-01 10:00:00','2023-08-01 10:00:00','2023-12-01 18:00:00'),('DEV004','烘箱','干燥','2023-04-01 11:00:00',NULL,'2023-09-01 17:00:00'),('DEV005','粘度计','测量','2023-05-01 12:00:00','2023-10-01 09:00:00',NULL),('DEV006','pH计','检测','2023-06-01 13:00:00','2023-11-01 08:00:00',NULL),('DEV007','天平','称量','2023-07-01 14:00:00',NULL,'2024-01-01 16:00:00'),('DEV008','分光光度计','分析','2023-08-01 15:00:00','2024-02-01 14:00:00',NULL),('DEV009','超声波清洗机','清洗','2023-09-01 16:00:00','2024-03-01 12:00:00',NULL),('DEV010','电导率仪','检测','2023-10-01 17:00:00',NULL,'2024-04-01 10:00:00'),('DEV011','旋转蒸发仪','浓缩','2023-11-01 18:00:00','2024-05-01 09:00:00',NULL),('DEV012','马弗炉','高温处理','2023-12-01 19:00:00',NULL,'2024-06-01 08:00:00'),('DEV013','冻干机','冷冻干燥','2024-01-01 20:00:00','2024-07-01 07:00:00',NULL),('DEV014','紫外可见分光光度计','光谱分析','2024-02-01 21:00:00',NULL,'2024-08-01 06:00:00'),('DEV015','气相色谱仪','分离分析','2024-03-01 22:00:00','2024-09-01 05:00:00',NULL),('DEV016','液相色谱仪','分离分析','2024-04-01 23:00:00',NULL,'2024-10-01 04:00:00'),('DEV017','质谱仪','成分分析','2024-05-01 00:00:00','2024-11-01 03:00:00',NULL),('DEV018','原子吸收光谱仪','元素分析','2024-06-01 01:00:00',NULL,'2024-12-01 02:00:00'),('DEV019','热重分析仪','热分析','2024-07-01 02:00:00','2025-01-01 01:00:00',NULL),('DEV020','差示扫描量热仪','热分析','2024-08-01 03:00:00',NULL,'2025-02-01 00:00:00'),('DEV021','粒度分析仪','粒径测量','2024-09-01 04:00:00','2025-03-01 23:00:00',NULL),('DEV022','流变仪','流变测量','2024-10-01 05:00:00',NULL,'2025-04-01 22:00:00'),('DEV023','显微镜','显微观察','2024-11-01 06:00:00','2025-05-01 21:00:00',NULL),('DEV024','电泳仪','电泳分析','2024-12-01 07:00:00',NULL,'2025-06-01 20:00:00'),('DEV025','PCR仪','基因扩增','2025-01-01 08:00:00','2025-07-01 19:00:00',NULL),('DEV026','培养箱','细胞培养','2025-02-01 09:00:00',NULL,'2025-08-01 18:00:00'),('DEV027','灭菌锅','灭菌','2025-03-01 10:00:00','2025-09-01 17:00:00',NULL),('DEV028','超净工作台','无菌操作','2025-04-01 11:00:00',NULL,'2025-10-01 16:00:00'),('DEV029','生物安全柜','生物防护','2025-05-01 12:00:00','2025-11-01 15:00:00',NULL),('DEV030','光谱仪','检测','2023-03-30 10:00:00',NULL,'2023-09-30 18:00:00');
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_device_after_insert` AFTER INSERT ON `device` FOR EACH ROW BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_device_after_update` AFTER UPDATE ON `device` FOR EACH ROW BEGIN
    DECLARE current_operator_no CHAR(8);
    SET current_operator_no = @current_user_id;
    IF current_operator_no IS NULL THEN
        SET current_operator_no = 'SYSTEM';
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_device_after_delete` AFTER DELETE ON `device` FOR EACH ROW BEGIN
    DECLARE current_operator_no CHAR(8);
    SET current_operator_no = @current_user_id;
    IF current_operator_no IS NULL THEN
        SET current_operator_no = 'SYSTEM';
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_material_after_insert` AFTER INSERT ON `material` FOR EACH ROW BEGIN
    DECLARE current_operator_no CHAR(8);
    SET current_operator_no = @current_user_id;
    IF current_operator_no IS NULL THEN
        SET current_operator_no = 'SYSTEM';
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_material_after_update` AFTER UPDATE ON `material` FOR EACH ROW BEGIN
    DECLARE current_operator_no CHAR(8);
    SET current_operator_no = @current_user_id;
    IF current_operator_no IS NULL THEN
        SET current_operator_no = 'SYSTEM';
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_material_after_delete` AFTER DELETE ON `material` FOR EACH ROW BEGIN
    DECLARE current_operator_no CHAR(8);
    SET current_operator_no = @current_user_id;
    IF current_operator_no IS NULL THEN
        SET current_operator_no = 'SYSTEM';
...
--
-- Dumping events for database 'devexp'
--

--
-- Dumping routines for database 'devexp'
--
...
-- Dump completed on 2025-06-12  4:35:44
