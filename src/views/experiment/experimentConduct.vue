            ...
            <div class="experiment-area">
                ...
                <div v-if="currentStage === 'prepare'" class="preparation-stage">
                    ...
                    <div class="image-gallery">
                        <img :src="staticImage1" alt="Preparation Image 1" class="experiment-image" />
                        <img :src="staticImage2" alt="Preparation Image 2" class="experiment-image" />
                    </div>
                    <a-button type="primary" @click="startExperiment" :loading="loading" class="start-button">
                        <template #icon>
                            <icon-play-arrow />
                        </template>
                ...
                <div v-else-if="currentStage === 'running'" class="running-stage">
                    ...
                    <div class="media-player">
                        <video v-if="mediaType === 'video'" ref="videoPlayer" :src="experimentVideo" controls autoplay
                            muted class="experiment-media" @ended="onMediaEnded">
                            Your browser does not support the video tag.
                        </video>
                        <img v-else-if="mediaType === 'gif'" :src="experimentGif" alt="Experiment GIF"
                            class="experiment-media" @load="onMediaLoaded" />
                        <a-spin v-if="loadingMedia" tip="加载中..."
                            style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);" />
                    </div>
                    <a-button type="text" status="danger" :loading="loading" @click="endExperimentImmediately"
                        class="end-button">
                        <!-- 修复 type="status" -->
                        <template #icon>
                            <icon-stop />
                ...
                <div v-else-if="currentStage === 'data_entry'" class="data-entry-stage">
                    ...
                    <a-form :model="experimentData" :label-col-props="{ span: 6 }" :wrapper-col-props="{ span: 18 }"
                        label-align="left" ref="experimentDataFormRef">
                        <a-row :gutter="16">
                            <a-col :span="12">
                                <a-form-item :label="$t('experiment.form.label.ExperimentNo')" field="ExperimentNo"
                                    :rules="[{ required: true, message: $t('experiment.form.validation.ExperimentNoRequired') }]">
                                    <a-input v-model="formDataExperimentNoComputed"
                            ...
                            <a-col :span="12">
                                <a-form-item :label="$t('experiment.form.label.MaterialCode')" field="MaterialCode"
                                    :rules="[{ required: true, message: $t('experiment.form.validation.MaterialCodeRequired') }]">
                                    <a-input v-model="materialCodeDataComputed"
                        ...
                        <a-col :span="12">
                            <a-form-item :label="$t('experiment.form.label.ProtocolNo')" field="ProtocolNo"
                                :rules="[{ required: true, message: $t('experiment.form.validation.ProtocolNoRequired') }]">
                                <a-input v-model="protocolNoDataComputed"
                                    :placeholder="$t('experiment.form.placeholder.ProtocolNo')" />
                            </a-form-item>
                        </a-col>
                        <a-col :span="12">
                            <a-form-item :label="$t('experiment.form.label.UserNo')" field="UserNo"
                                :rules="[{ required: true, message: $t('experiment.form.validation.UserNoRequired') }]">
                                <a-input v-model="formDataUserNoComputed"
                    ...
                    <a-row :gutter="16">
                        <a-col :span="12">
                            <a-form-item :label="$t('experiment.form.label.HeatError')" field="HeatError"
                                ...
                                <a-input-number v-model="heatErrorDataComputed" :step="0.01" allow-clear
                        ...
                        <a-col :span="12">
                            <a-form-item :label="$t('experiment.form.label.MixError')" field="MixError"
                                ...
                                <a-input-number v-model="mixErrorDataComputed" :step="0.01" allow-clear
                    ...
                    <a-row :gutter="16">
                        <a-col :span="12">
                            <a-form-item :label="$t('experiment.form.label.StartTime')" field="StartTime"
                                ...
                                <a-time-picker v-model="startTimeDataComputed" format="HH:mm:ss" value-format="HH:mm:ss"
                                    allow-clear :placeholder="$t('experiment.form.placeholder.StartTime')" />
                            </a-form-item>
                        </a-col>
                        <a-col :span="12">
                            <a-form-item :label="$t('experiment.form.label.EndTime')" field="EndTime"
                                ...
                                <a-time-picker v-model="endTimeDataComputed" format="HH:mm:ss" value-format="HH:mm:ss"
                                    allow-clear :placeholder="$t('experiment.form.placeholder.EndTime')" />
                            </a-form-item>
                        </a-col>
                    </a-row>
                    <a-form-item :label="$t('experiment.form.label.GelTime')" field="GelTime">
                        <a-input v-model="gelTimeDataComputed" disabled />
                    </a-form-item>

                    <a-button type="primary" :loading="loading" @click="uploadExperimentData" class="upload-button">
...
const createNumberComputed = (key: 'HeatError' | 'MixError') => computed({
    ...
    set: (value: number | undefined) => {
        (experimentData[key] as number | null | undefined) = value === undefined ? null : value;
    }
});
...
const createStringComputed = (key: 'ExperimentNo' | 'MaterialCode' | 'ProtocolNo' | 'UserNo') => computed({
    get: () => {
        const value = experimentData[key];
        return value === null ? undefined : (value as string | undefined);
    },
    set: (value: string | undefined) => {
        (experimentData[key] as string | null | undefined) = (value === undefined || value === '') ? null : value;
    }
});
...
const createTimeComputed = (key: 'StartTime' | 'EndTime') => computed({
    ...
    set: (value: string | undefined) => {
        experimentData[key] = (value === undefined || value === '') ? null : value;
    }
});
...
const startExperiment = () => {
    ...
    if (mediaType.value === 'video') {
        nextTick(() => { // <-- 关键：等待 DOM 更新后才尝试访问 videoPlayer.value
...
const uploadExperimentData = async () => {
    ...
    if (!errors) {
        ...
        try {
            ...
            if (dataToSend.ExperimentNo === '') dataToSend.ExperimentNo = null as any;
            ...
            Message.error(`${t('experiment.conduct.uploadFail')}: ${error.message || '未知错误'}`);
        } finally {
            setLoading(false);
        }
    } else {
