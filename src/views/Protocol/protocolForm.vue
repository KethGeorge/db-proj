<template>
    <div class="container">
        <Breadcrumb
            :items="['protocol', isCreate ? 'protocol.form.title.create' : (isEdit ? 'protocol.form.title.edit' : 'protocol.form.title.view')]" />
        <a-form ref="formRef" layout="vertical" :model="formData" @submit="onSubmitClick">
            <a-space direction="vertical" :size="16">
                <a-card class="general-card" :bordered="false">
                    ...
                    <a-row :gutter="16">
                        <a-col :span="8">
                            <a-form-item :label="$t('protocol.form.label.ProtocolNo')" field="ProtocolNo"
                                :rules="[{ required: true, message: $t('protocol.form.validation.ProtocolNoRequired') }, { min: 1, max: 8, message: $t('protocol.form.validation.ProtocolNoLength') }]">
                                <a-input v-model="formData.ProtocolNo"
                                    :placeholder="$t('protocol.form.placeholder.ProtocolNo')" :disabled="!isCreate" />
                            </a-form-item>
                        </a-col>
                        <a-col :span="8">
                            <a-form-item :label="$t('protocol.form.label.MaterialCode')" field="MaterialCode"
                                :rules="[{ required: true, message: $t('protocol.form.validation.MaterialCodeRequired') }, { min: 1, max: 16, message: $t('protocol.form.validation.MaterialCodeLength') }]">
                        ...
                        <a-col :span="8">
                            <a-form-item :label="$t('protocol.form.label.UserNo')" field="UserNo"
                                :rules="[{ required: true, message: $t('protocol.form.validation.UserNoRequired') }, { min: 1, max: 8, message: $t('protocol.form.validation.UserNoLength') }]">
                                <a-input v-model="formData.UserNo" :placeholder="$t('protocol.form.placeholder.UserNo')"
                                    :disabled="!isEdit && !isCreate" />
                            </a-form-item>
                        </a-col>
                    </a-row>
                    ...
                    <a-row :gutter="16">
                        <a-col :span="8">
                            <a-form-item :label="$t('protocol.form.label.NSN')" field="NSN"
                                :rules="[{ max: 20, message: $t('protocol.form.validation.NSNLength') }]">
                                <a-input v-model="formData.NSN" :placeholder="$t('protocol.form.placeholder.NSN')"
                                    :disabled="!isEdit && !isCreate" />
                            </a-form-item>
                        </a-col>
                        <a-col :span="8">
                            <a-form-item :label="$t('protocol.form.label.SHT')" field="SHT"
                                :rules="[{ type: 'number', message: $t('protocol.form.validation.SHTNumber') }]">
                                <a-input-number v-model="shtComputed" :placeholder="$t('protocol.form.placeholder.SHT')"
                                    :disabled="!isEdit && !isCreate" :step="0.1" allow-clear />
                            </a-form-item>
                        </a-col>
                        <a-col :span="8">
                            <a-form-item :label="$t('protocol.form.label.SMS')" field="SMS"
                                :rules="[{ type: 'number', message: $t('protocol.form.validation.SMSNumber') }]">
                                <a-input-number v-model="smsComputed" :placeholder="$t('protocol.form.placeholder.SMS')"
                                    :disabled="!isEdit && !isCreate" :step="0.1" allow-clear />
                            </a-form-item>
                        </a-col>
                    </a-row>
                    ...
                    <a-row :gutter="16">
                        <a-col :span="8">
                            <a-form-item :label="$t('protocol.form.label.MixingAngle')" field="MixingAngle"
                                :rules="[{ type: 'number', message: $t('protocol.form.validation.MixingAngleNumber') }]">
                                <a-input-number v-model="mixingAngleComputed"
                                    :placeholder="$t('protocol.form.placeholder.MixingAngle')"
                                    :disabled="!isEdit && !isCreate" :step="0.1" allow-clear />
                            </a-form-item>
                        </a-col>
                        <a-col :span="8">
                            <a-form-item :label="$t('protocol.form.label.MixingRadius')" field="MixingRadius"
                                :rules="[{ type: 'number', message: $t('protocol.form.validation.MixingRadiusNumber') }]">
                                <a-input-number v-model="mixingRadiusComputed"
                                    :placeholder="$t('protocol.form.placeholder.MixingRadius')"
                                    :disabled="!isEdit && !isCreate" :step="0.1" allow-clear />
                            </a-form-item>
                        </a-col>
                        <a-col :span="8">
                            <a-form-item :label="$t('protocol.form.label.MeasurementInterval')"
                                field="MeasurementInterval"
                                :rules="[{ type: 'number', message: $t('protocol.form.validation.MeasurementIntervalNumber') }]">
                                <a-input-number v-model="measurementIntervalComputed"
                                    :placeholder="$t('protocol.form.placeholder.MeasurementInterval')"
                                    :disabled="!isEdit && !isCreate" :step="0.1" allow-clear />
                            </a-form-item>
                        </a-col>
                    </a-row>
...
import { queryProtocolDetail, createProtocol, updateProtocolInfo, ProtocolRecord } from '@/api/protocol';
...
const protocolNoFromRoute = computed(() => route.params.protocolNo as string | undefined);
...
const createNumberComputed = (key: keyof ProtocolRecord) => computed({
    get: () => {
...
const measurementIntervalComputed = createNumberComputed('MeasurementInterval');
...
const fetchProtocolData = async (protocolNo: string) => {
    ...
    try {
        ...
        Message.error(`${t('protocol.message.fetchFail')}: ${error.message || '未知错误'}`);
    ...
    if (!errors) {
        ...
        try {
            ...
            const nullableNumberFields: (keyof ProtocolRecord)[] = [
                'SHT', 'SMS', 'MixingAngle', 'MixingRadius', 'MeasurementInterval'
            ];
