  ...
  <div class="container">
    <Breadcrumb
      :items="[
        'Material',
        isCreate
          ? 'material.form.title.create'
          : isEdit
          ? 'material.form.title.edit'
          : 'material.form.title.view',
      ]"
    />
    <a-form
      ref="formRef"
      ...
      <a-space direction="vertical" :size="16">
        <a-card class="general-card" :bordered="false">
          ...
          <a-row :gutter="16">
            <a-col :span="12">
              <a-form-item
                :label="$t('material.form.label.MaterialCode')"
                field="MaterialCode"
                :rules="[
                  {
                    required: true,
                    message: $t(
                      'material.form.validation.MaterialCodeRequired'
                    ),
                  },
                  {
                    min: 1,
                    max: 16,
                    message: $t('material.form.validation.MaterialCodeLength'),
                  },
                ]"
              >
                <a-input
                  v-model="formData.MaterialCode"
                  :placeholder="$t('material.form.placeholder.MaterialCode')"
                  :disabled="!isCreate"
                />
            ...
            <a-col :span="12">
              <a-form-item
                :label="$t('material.form.label.MaterialName')"
                field="MaterialName"
                :rules="[
                  {
                    required: true,
                    message: $t(
                      'material.form.validation.MaterialNameRequired'
                    ),
                  },
                  {
                    min: 1,
                    max: 20,
                    message: $t('material.form.validation.MaterialNameLength'),
                  },
                ]"
              >
                <a-input
...
<script lang="ts" setup>
  ...
  const materialCodeFromRoute = computed(
    () => route.params.materialCode as string | undefined
  );
  ...
  const fetchMaterialData = async (materialCode: string) => {
    ...
    try {
      ...
      Message.error(
        `${t('material.message.fetchFail')}: ${error.message || '未知错误'}`
      );
