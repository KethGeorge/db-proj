<template>
    <div class="container">
        ...
        <a-card class="general-card" :title="$t('experiment.list')">
            ...
            <a-table row-key="ExperimentNo" :loading="loading" :pagination="pagination"
                ...
                <template #operations="{ record }">
                    <a-space :size="8">
                        <a-button type="text" size="small" @click="handleView(record.ExperimentNo)">
                            {{ $t('experiment.columns.operations.view') }}
                        </a-button>
                        <a-button v-permission="['admin']" type="text" size="small" @click="handleEdit(record.ExperimentNo)">
                            {{ $t('experiment.columns.operations.edit') }}
                        </a-button>
                        <a-popconfirm @ok="handleDelete(record.ExperimentNo)" :content="$t('experiment.message.confirmDelete')">
...
import { queryExperimentList, deleteExperiment, ExperimentRecord, ExperimentParams } from '@/api/experiment';
...
const fetchData = async (params: ExperimentParams = { current: 1, pageSize: 20 }) => {
    setLoading(true);
    try {
...
const handleConductExperiment = () => { // 新增的执行实验按钮处理
    router.push({
...
const handleDelete = async (experimentNo: string | undefined) => {
    ...
    try {
        ...
        Message.error(`${t('experiment.message.deleteFail')}: ${error.message || '未知错误'}`);
    } finally {
        setLoading(false);
    }
};
