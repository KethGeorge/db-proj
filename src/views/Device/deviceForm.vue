    ...
    <Breadcrumb
      :items="[
        ...
        isCreate
          ...
          : isEdit
          ? 'device.form.title.edit'
          : 'device.form.title.view',
      ]"
    />
    <a-form
      ref="formRef"
      ...
      <a-space direction="vertical" :size="16">
        <a-card class="general-card" :bordered="false">
          ...
          <a-row :gutter="16">
            <a-col :span="12">
              <a-form-item
                :label="$t('device.form.label.DeviceNo')"
                field="DeviceNo"
                :rules="[
                  {
                    required: true,
                    message: $t('device.form.validation.DeviceNoRequired'),
                  },
                  {
                    min: 1,
                    max: 20,
                    message: $t('device.form.validation.DeviceNoLength'),
                  },
                ]"
              >
                <a-input
            ...
            <a-col :span="12">
              <a-form-item
                :label="$t('device.form.label.DeviceName')"
                field="DeviceName"
                :rules="[
                  {
                    required: true,
                    message: $t('device.form.validation.DeviceNameRequired'),
                  },
                  {
                    min: 1,
                    max: 8,
                    message: $t('device.form.validation.DeviceNameLength'),
                  },
                ]"
              >
                <a-input
          ...
          <a-row :gutter="16">
            <a-col :span="12">
              <a-form-item
                :label="$t('device.form.label.DeviceUsage')"
                field="DeviceUsage"
                :rules="[
                  {
                    max: 10,
                    message: $t('device.form.validation.DeviceUsageLength'),
                  },
                ]"
              >
                <a-input
          ...
          <a-row :gutter="16">
            <a-col :span="8">
              <a-form-item
                :label="$t('device.form.label.DStartTime')"
                field="DStartTime"
              >
                <a-date-picker
                  v-model="formData.DStartTime"
                  show-time
                  style="width: 100%"
            ...
            <a-col :span="8">
              <a-form-item :label="$t('device.form.label.DMT')" field="DMT">
                <a-date-picker
                  v-model="formData.DMT"
                  show-time
                  style="width: 100%"
                  :placeholder="$t('device.form.placeholder.DMT')"
                  :disabled="!isEdit && !isCreate"
                  value-format="YYYY-MM-DDTHH:mm:ss"
                />
              </a-form-item>
            ...
            <a-col :span="8">
              <a-form-item
                :label="$t('device.form.label.DStopTime')"
                field="DStopTime"
              >
                <a-date-picker
                  v-model="formData.DStopTime"
                  show-time
                  style="width: 100%"
...
<script lang="ts" setup>
  ...
  const deviceNoFromRoute = computed(
    () => route.params.deviceNo as string | undefined
  );
  ...
  const fetchDeviceData = async (deviceNo: string) => {
    ...
    try {
      ...
      Message.error(
        `${t('device.message.fetchFail')}: ${error.message || '未知错误'}`
      );
  ...
  const resetForm = () => {
    ...
    } else if (deviceNoFromRoute.value) {
